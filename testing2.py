...
try:
    data = pd.read_csv('UrbanPopulation.csv')
except FileNotFoundError:
    print("Error: Could not find 'UrbanPopulation.csv' file. Please check the file path.")
    exit()
...
if data.empty:
    print("Error: 'UrbanPopulation.csv' file seems empty. Please ensure it contains data.")
    exit()

# Scatter plot of year vs ForestA emissions
plt.figure(figsize=(10, 6))
plt.scatter(data['year'], data['UrbanPopulation'], color='blue', label='Actual Data')

# Features and target variable
X = data[['year']]
y = data['UrbanPopulation']
...
for year, emission in zip(future_years, future_predictions):
    print(f"Year: {year[0]}, Predicted Urban Population: {emission}")

plt.xlabel('Year')
plt.ylabel('Urban Population')
plt.title('Urban Population Over Time with Polynomial Regression Predictions')
plt.legend()
plt.grid(True)
plt.show()  
